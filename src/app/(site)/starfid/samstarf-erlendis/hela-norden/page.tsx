...
export const metadata: Metadata = { title: "Hela norden skal leva", description: "Norrænt samstarfsverkefni um líflegt og blómlegt dreifbýli um allt Norðurlönd." };

export default async function Page() {
  const page = await client.fetch(pageByIdQuery, { id: "page-starfid-hela-norden" }).catch(() => null);
  return (
    <StarfidLayout title={page?.title || "Hela norden skal leva"} section="samstarf-erlendis" heroImage={page?.heroImage}>
      {page?.body ? <PortableTextRenderer value={page.body} /> : (
        <div>
          <blockquote className="border-l-4 pl-4 italic text-gray-600 mb-6" style={{ borderColor: "var(--teal)" }}>
          ...
          <h2 className="text-xl font-bold mb-3" style={{ color: "var(--navy)" }}>Fundarseta</h2>
          <p className="text-gray-600 leading-relaxed mb-6">
            Tveir fundir eru haldnir árlega. Voarfundirnir skiptast á milli Norðurlandanna.
          </p>
          <h2 className="text-xl font-bold mb-3" style={{ color: "var(--navy)" }}>Þátttaka LBL</h2>
          <p className="text-gray-600 leading-relaxed mb-6">
